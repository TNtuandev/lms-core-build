...
export default function StudyCode({
  ...
  const [currentCode, setCurrentCode] = useState(
    initValue?.sampleContent ?? exercise.initialCode,
  );
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data && event.data.language === "java") {
        ...
        if (event.data.files && event.data.files[0]) {
          console.log(
            "Updating currentCode with:",
            event.data.files[0].content,
          );
      ...
      if (event.data && event.data.eventType) {
        switch (event.data.eventType) {
          ...
          case "codeChange":
            ...
            if (event.data.files && event.data.files[0]) {
              console.log(
                "Updating currentCode from codeChange:",
                event.data.files[0].content,
              );
              setCurrentCode(event.data.files[0].content);
            }
          ...
          case "codeResponse":
            ...
            if (event.data.files && event.data.files[0]) {
              console.log(
                "Updating currentCode from response:",
                event.data.files[0].content,
              );
              setCurrentCode(event.data.files[0].content);
            }
    ...
    <div className="flex bg-[#1e1e1e] h-screen">
      ...
      <div className="w-[380px] bg-[#2d2d30] border-r border-gray-600 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {activeTab === "goi-y" && (
            <div className="p-4">
              <p className="text-sm text-white">{initValue?.suggestion}</p>
            </div>
          )}
              ...
              {showAnswer || failedAttempts >= 3 ? (
                ...
                <p className="text-sm text-white">
                  ...
                  <span className="text-yellow-400">
                    Số lần đã thử: {failedAttempts}/3
                  </span>
                </p>
              )}
            </div>
                    ...
                    <div>
                      ...
                      {isRunning ? (
                        ...
                        <p className="text-gray-400">
                          Chưa có kết quả. Nhấn &#34;Kiểm tra&#34; để nộp bài.
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      <div>
                        ...
                        <span className="ml-2 text-white">
                          {new Date(
                            submissionData.code.submittedAt,
                      ...
                      {submissionData.code.runtimeMs && (
                        <div>
                          ...
                          <span className="ml-2 text-white">
                            {submissionData.code.runtimeMs}ms
                          </span>
                        </div>
                      )}
                    </div>
